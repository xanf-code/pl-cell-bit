...
function Sidebar() {
    ...
    return (
        <div className={`w-[70%] md:w-[20%] overflow-hidden bg-white shadow-xl text-gray-500 overflow-y-scroll scrollbar-hide fixed inset-y-0 left-0 transform ${!isSidebarOpen && '-translate-x-full'} transition duration-200 ease-in-out`}>
            ...
            <div className='px-5'>
                {session && <div onClick={() => signOut({ callbackUrl: "/" })} className="inline-flex items-center justify-center px-5 py-3 text-base font-medium text-center text-indigo-100 border border-indigo-500 rounded-lg shadow-sm cursor-pointer hover:text-white bg-gradient-to-br from-purple-500 via-indigo-500 to-indigo-500 w-full">
